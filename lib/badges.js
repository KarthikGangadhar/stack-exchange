...
module.exports = class Badges extends stackExchange {
    ...
    badges(options = {}, callback) {
        let url = helper.getEndpoint(options , "BADGES");
    ...
    badge_by_tags(options = {}, callback) {
        let url = helper.getEndpoint(options , "BADGES_BY_TAGS");        
        helper.getResponse(url).then((response) => {
            callback(response);
        }).catch((err) => {
            callback({
    ...
    badge_recipients(options = {}, callback) {
        let url = helper.getEndpoint(options , "BADGES_RECIPIENTS");        
        helper.getResponse(url).then((response) => {
            callback(response);
        }).catch((err) => {
            callback({
    ...
    badge_recipients_by_id(id, options = {}, callback) {
        let url = helper.getEndpoint(options , "BADGES_RECIPIENTS_BY_ID");        
    ...
    badges_by_id(id, options = {}, callback) {
        let url = helper.getEndpoint(options , "BADGES_BY_ID");        
        let formated_url = util.format(url, id);
        helper.getResponse(formated_url).then((response) => {
            callback(response);
        }).catch((err) => {
    ...
    badges_by_name(name, options = {}, callback) {
        options["inname"] = name || ""
        let url = helper.getEndpoint(options , "BADGES_BY_NAME");        
        helper.getResponse(url).then((response) => {
            callback(response);
        }).catch((err) => {
            callback({
