...
module.exports = class Users extends stackExchange {
    ...
    comments_by_users_to_user(user_id, to_id, options = {}, callback) {
        ...
        let formatted_url = util.format(url, user_id, to_id);
        helper.getResponse(formatted_url).then((response) => {
            callback(response);
        }).catch((err) => {
            callback({
    ...
    users_network_activity(ids, options = {}, callback) {
        let url = helper.getEndpoint(this.version, options, 'USERS_NETWORK_ACTIVITY');
        let formatted_url = util.format(url, ids);
        helper.getResponse(formatted_url).then((response) => {
            callback(response);
        }).catch((err) => {
    ...
    top_user_answers_in_tags(ids, tags, options = {}, callback) {
        ...
        let formatted_url = util.format(url, ids, tags);
        helper.getResponse(formatted_url).then((response) => {
            callback(response);
        }).catch((err) => {
            callback({
